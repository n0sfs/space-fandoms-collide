...
## ✅ Testing
An opt-in self-test harness ships alongside the game: open `index.html?test=1` in a browser to run ~94 automated checks (rendering, the full difficulty/level sweep, boss rotation fairness, the perk system, Boss Rush and Daily Challenge determinism, achievement unlock paths, profile isolation, and more) and see a visual pass/fail report. It only loads with that query flag, so it has zero effect on normal play, and every destructive check runs under a disposable pilot profile that's deleted when the run finishes.
