...
* **Hostile AI:** Dodge tracking fire from TIE Advanced fighters and homing Sentinels — now a beveled hunter-drone with a pulsing sensor eye, an expanding radar-scan ring, and thruster glow at each scuttling foot — whose chase speed is capped against whatever hull you picked so disengaging is always possible.
* **One Consistent Light Source:** Every ship, boss, and asteroid in the 2D fight is now lit from the same fixed direction — a hard rim light on the sunlit edge, a contact-shadow falloff on the far side, and independently-blinking hazard lights on ships and bosses — instead of each design drawing its own self-contained glow. The same lighting carries into the 3D cockpit pursuit, so a boss looks like the same object across both phases of the fight.
* **Boulders & Ore Veins:** The biggest asteroids in a field now take two hits to crack open — the first chips it with a visible stress fracture across the rock, the second splits it — instead of every rock popping in one shot regardless of size. A rare mineral-rich asteroid (glowing veins pulsing across its surface) pays a guaranteed, larger scrap bonus if you spot it and prioritize it.
* **Elite Fighters:** A rare (5%) tougher variant of every common fighter and sentinel — gold rim light, gold hazard lights, two hits to bring down, a bigger score and scrap payout on the kill. The same "worth noticing" design as an ore asteroid, applied to the ship roster.
* **Fleet Battles:** Every 6th filler level becomes a large-scale engagement — two wingmen fight alongside you (their kills count for your score), a denser enemy wave than a normal patrol, and a pair of capital ships trading beam fire in the dim background purely for atmosphere. Wingmen can be shot down by enemy fire, so it's real backup, not a freebie.
* **A Boss Shows Its Wounds:** A boss in the open 2D fight now trails sparks and smoke that intensify as it takes damage, building visibly toward the moment it breaks and runs for the asteroid field — instead of looking pristine right up until it flees.
* **Two-Phase Boss Fights:** Wound a boss past half health and it breaks for a nearby asteroid field. You follow it into the cockpit and finish the fight in 3D, its damage carried across — dodging rocks and escort fighters while it holds a firing distance and weaves, trailing smoke and embers the closer it gets to dying. Every landed hit pops a small impact burst; the killing blow sets off a proper chained explosion — a bright fireball core, secondary flares a beat behind it, a scatter of embers, and a smoke cloud left hanging where it died. It's lit from a fixed direction with a hard rim light and a contact-shadow halo, rather than the flat self-lit look every other hull gets, so it reads as an actual mass rather than a sprite. The swarm bosses have no single thing to chase, so they stay a 2D fight start to finish.
...
* **Hyperspace Anomalies:** Every 7th level no longer cuts straight to the cockpit — it opens with a short ordinary flight through a lighter asteroid/fighter patrol while a spatial rift visibly forms and grows in the field, pulling in debris as it builds. Once it's ready, it pulls the ship in and phases the fight into that cockpit for a rail-shooter round: evade and survive against the clock.
...
* **Responsive Touch Controls:** Automatically detects mobile devices and renders an on-screen joystick, fire button, and nuke detonator. The Flight Manual swaps to describe the joystick/buttons instead of mouse-and-keyboard on a touch device, so a phone player isn't told to use keys that aren't there.
* **A Collapsible Flight Manual:** Tap to expand for controls and the powerup legend, collapsed by default — the menu is immediately visible on first load instead of sitting behind a permanently-expanded reference panel, on both mobile and desktop.
...
* **Dimensional Shading:** Every ship, enemy, asteroid, and satellite carries richer gradient shading, rim lighting, and specular highlights for a more three-dimensional look, and the ship selector shows at-a-glance Speed/Fire Rate bars for every ship. TIE fighters now show their engine glow in flight, and your own ship holds a gentle idle float while coasting that settles out the moment you're back on the throttle.
...
## ✅ Testing
An opt-in self-test harness ships alongside the game: open `index.html?test=1` in a browser to run ~112 automated checks (rendering, the full difficulty/level sweep, boss rotation fairness, the perk system, Boss Rush and Daily Challenge determinism, achievement unlock paths, profile isolation, DPR-aware aim mapping, the hyperspace approach-to-anomaly phase-in, and more) and see a visual pass/fail report. It only loads with that query flag, so it has zero effect on normal play, and every destructive check runs under a disposable pilot profile that's deleted when the run finishes.
