...
function spawnParticles(x, y, color, count, speedMod = 1) {
    for(let i=0; i<count; i++) {
        let speed = (Math.random() * 6 + 2) * speedMod;
...
diffButtons.forEach(btn => { 
    const diffAction = (e) => { e.preventDefault(); initAudio(); gameDifficulty = btn.dataset.diff; updateDifficultyUI(); };
...
function triggerNuke() {
    ...
    enemyBullets = []; 
    targets.forEach(t => { if (t.hp !== undefined) t.hp -= 50; else t.hp = -1; });
    updateUI();
}
...
window.addEventListener("keydown", (e) => {
    ...
    if ((gameState === "GAMEOVER" || gameState === "VICTORY") && e.code === "KeyR") { if(menuOverlay) menuOverlay.classList.remove("hidden"); gameState = "MENU"; }
});
...
function loadLeaderboard() {
    try { let stored = localStorage.getItem("starWarsAsteroidsScores"); if (stored) highScores = JSON.parse(stored); } catch(e) {}
...
function checkAndSaveScore() { highScores.push({ name: currentPlayerName, score: score }); highScores.sort((a, b) => b.score - a.score); highScores = highScores.slice(0, 5); try { localStorage.setItem("starWarsAsteroidsScores", JSON.stringify(highScores)); } catch(e) {} loadLeaderboard(); }
loadLeaderboard();
...
// --- SHIPS & STATS ---
const ShipDesigns = {
    xwing: { name: "X-WING", laserColor: "#ff3333", stats: { thrust: 6, fric: 0.98, fireRate: 0.25 },
        fire: () => { bullets.push(createBolt(0)); playSfx('shoot'); },
        draw: (ctx, r, thrusting) => { 
            let hullGrad = ctx.createLinearGradient(-r, 0, r, 0); hullGrad.addColorStop(0, "#778899"); hullGrad.addColorStop(1, "#f8f8ff");
            ctx.fillStyle = "#8a9ea8"; ctx.strokeStyle = "#444"; ctx.lineWidth = 1;
            ...
            ctx.fillStyle = "#333"; ctx.fillRect(-r*1.1, -r*0.6, r*0.3, r*0.2); ctx.fillRect(-r*1.1, r*0.4, r*0.3, r*0.2);
            ...
            ctx.fillStyle = hullGrad; ctx.strokeStyle = "#222"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(r*1.4, 0); ctx.lineTo(r/2, -r/4); ctx.lineTo(-r, -r/4); ctx.lineTo(-r, r/4); ctx.lineTo(r/2, r/4); ctx.fill(); ctx.stroke();
    ...
    falcon: { name: "FALCON", laserColor: "#ff3333", stats: { thrust: 5, fric: 0.97, fireRate: 0.35 },
        fire: () => { bullets.push(createBolt(0)); bullets.push(createBolt(Math.PI)); playSfx('shoot'); }, 
        draw: (ctx, r, thrusting) => {
            let hullGrad = ctx.createRadialGradient(0, 0, 0, 0, 0, r); hullGrad.addColorStop(0, "#e0e0e0"); hullGrad.addColorStop(1, "#888888");
            ctx.fillStyle = hullGrad; ctx.strokeStyle = "#333"; ctx.lineWidth = 1;
            ...
            ctx.fillStyle = "#999"; ctx.beginPath(); ctx.moveTo(-r/5, r*0.7); ctx.lineTo(r*0.5, r*0.8); ctx.lineTo(r*0.5, r*0.6); ctx.fill(); ctx.stroke();
            ctx.beginPath(); ctx.arc(r*0.6, r*0.75, r*0.25, 0, Math.PI*2); ctx.fill(); ctx.stroke(); ctx.fillStyle = "#112233"; ctx.beginPath(); ctx.arc(r*0.65, r*0.75, r*0.1, 0, Math.PI*2); ctx.fill(); 
            ctx.fillStyle = "#777"; ctx.beginPath(); ctx.arc(-r*0.4, -r*0.4, r*0.2, 0, Math.PI); ctx.fill(); ctx.stroke();
            if (thrusting) { applyGlow(ctx, "#33ccff", 20); ctx.fillStyle = "#e6ffff"; ctx.fillRect(-r*1.1, -r*0.5, 6 + Math.random()*4, r); clearGlow(ctx); }
        }
    },
    tiefighter: { name: "TIE FIGHTER", laserColor: "#33ff33", stats: { thrust: 8, fric: 0.99, fireRate: 0.15 },
        ...
        draw: (ctx, r, thrusting) => {
            ...
            ctx.strokeStyle = "#555"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(0, -r/2); ctx.lineTo(0, -r*1.2); ctx.stroke(); ctx.beginPath(); ctx.moveTo(0, r/2); ctx.lineTo(0, r*1.2); ctx.stroke();
            let panGrad = ctx.createLinearGradient(0, -r*1.2, 0, r*1.2); panGrad.addColorStop(0,"#111"); panGrad.addColorStop(0.5,"#333"); panGrad.addColorStop(1,"#111");
    ...
    enterprise: { name: "ENTERPRISE", laserColor: "#33ccff", stats: { thrust: 4, fric: 0.95, fireRate: 0.55 },
        fire: () => { let b = createBolt(0, 15); b.r = 6; bullets.push(b); playSfx('shoot'); }, 
        draw: (ctx, r, thrusting) => {
            let hullGrad = ctx.createRadialGradient(r*0.5, 0, 0, r*0.5, 0, r*0.8); hullGrad.addColorStop(0, "#fff"); hullGrad.addColorStop(1, "#b0bec5");
            ctx.fillStyle = "#cfd8dc"; ctx.strokeStyle = "#78909c"; ctx.lineWidth = 1; ctx.beginPath(); ctx.ellipse(-r*0.3, 0, r*0.7, r*0.25, 0, 0, Math.PI*2); ctx.fill(); ctx.stroke();
            ctx.fillStyle = "#ffcc00"; ctx.beginPath(); ctx.ellipse(-r*0.9, 0, r*0.1, r*0.2, 0, 0, Math.PI*2); ctx.fill();
            ctx.fillStyle = "#90a4ae"; ctx.fillRect(-r*1.1, -r, r*1.4, r*0.3); ctx.strokeRect(-r*1.1, -r, r*1.4, r*0.3); ctx.fillRect(-r*1.1, r*0.7, r*1.4, r*0.3); ctx.strokeRect(-r*1.1, r*0.7, r*1.4, r*0.3);
    ...
    apollo: { name: "APOLLO", laserColor: "#ffaa00", stats: { thrust: 9, fric: 1.0, fireRate: 0.25 }, 
        fire: () => { bullets.push(createBolt(0)); playSfx('shoot'); },
        draw: (ctx, r, thrusting) => {
            let bodyGrad = ctx.createLinearGradient(0, -r*0.4, 0, r*0.4); bodyGrad.addColorStop(0, "#999"); bodyGrad.addColorStop(0.5, "#fff"); bodyGrad.addColorStop(1, "#999");
            ctx.fillStyle = bodyGrad; ctx.strokeStyle = "#555"; ctx.lineWidth = 1; ctx.fillRect(-r*1.2, -r*0.4, r*1.4, r*0.8); ctx.strokeRect(-r*1.2, -r*0.4, r*1.4, r*0.8);
            ctx.fillStyle = "#d32f2f"; ctx.fillRect(-r*0.4, -r*0.4, r*0.1, r*0.8); ctx.fillStyle = "#1976d2"; ctx.fillRect(r*0.2, -r*0.4, r*0.05, r*0.8);
            ctx.fillStyle = "#ddd"; ctx.beginPath(); ctx.moveTo(r*0.2, -r*0.4); ctx.lineTo(r*1.2, 0); ctx.lineTo(r*0.2, r*0.4); ctx.fill(); ctx.stroke();
    ...
    serenity: { name: "SERENITY", laserColor: "#ffaa00", stats: { thrust: 7, fric: 0.98, fireRate: 0.25 },
        fire: () => { bullets.push(createBolt(0)); playSfx('shoot'); },
        draw: (ctx, r, thrusting) => {
            ctx.fillStyle = "#8ba8a0"; ctx.strokeStyle = "#333"; ctx.lineWidth = 1; ctx.beginPath(); ctx.ellipse(-r*0.2, 0, r*0.6, r*0.25, 0, 0, Math.PI*2); ctx.fill(); ctx.stroke();
            ...
            ctx.fillStyle = "#ffdd88"; ctx.fillRect(r*0.85, -r*0.05, r*0.1, r*0.1); ctx.fillStyle = "#556655"; ctx.fillRect(-r*0.1, -r*0.6, r*0.35, r*0.4); ctx.fillRect(-r*0.1, r*0.2, r*0.35, r*0.4);
    ...
    borg: { name: "BORG CUBE", laserColor: "#00ff00", stats: { thrust: 3, fric: 0.90, fireRate: 1.5 }, 
        fire: () => { for(let i=0; i<Math.PI*2; i+=Math.PI*2/5) bullets.push(createBolt(i, 8)); playSfx('shoot'); }, 
        draw: (ctx, r, thrusting) => {
            let cubeGrad = ctx.createRadialGradient(0, 0, 0, 0, 0, r); cubeGrad.addColorStop(0, "#222"); cubeGrad.addColorStop(1, "#050505");
            ctx.fillStyle = cubeGrad; ctx.strokeStyle = "#004400"; ctx.lineWidth = 1; ctx.fillRect(-r*0.8, -r*0.8, r*1.6, r*1.6); ctx.strokeRect(-r*0.8, -r*0.8, r*1.6, r*1.6);
    ...
    pelican: { name: "PELICAN", laserColor: "#00aaff", stats: { thrust: 5, fric: 0.96, fireRate: 0.65 },
        fire: () => { bullets.push(createBolt(0)); bullets.push(createBolt(-0.15)); bullets.push(createBolt(0.15)); playSfx('shoot'); }, 
        draw: (ctx, r, thrusting) => {
            let camoGrad = ctx.createLinearGradient(-r, 0, r, 0); camoGrad.addColorStop(0, "#3a4015"); camoGrad.addColorStop(1, "#5b6329");
            ctx.fillStyle = camoGrad; ctx.strokeStyle = "#222"; ctx.lineWidth = 1; ctx.fillRect(-r*0.8, -r*0.3, r*1.6, r*0.6); ctx.strokeRect(-r*0.8, -r*0.3, r*1.6, r*0.6);
    ...
    tardis: { name: "TARDIS", laserColor: "#ffffff", stats: { thrust: 6, fric: 0.99, fireRate: 1.0 },
        fire: () => { let b = createBolt(0, 8); b.r = 20; b.range = canvas.width*0.4; bullets.push(b); playSfx('shoot'); }, 
        draw: (ctx, r, thrusting) => {
            ctx.fillStyle = "#003b6f"; ctx.strokeStyle = "#001122"; ctx.lineWidth = 1.5; ctx.fillRect(-r*0.7, -r*0.7, r*1.4, r*1.4); ctx.strokeRect(-r*0.7, -r*0.7, r*1.4, r*1.4); ctx.strokeRect(-r*0.5, -r*0.5, r*1.0, r*1.0);
            ...
            if (thrusting) { applyGlow(ctx, "#0088ff", 25); ctx.strokeStyle = "#0088ff"; ctx.lineWidth = 3; ctx.strokeRect(-r*0.8, -r*0.8, r*1.6, r*1.6); clearGlow(ctx); }
        }
    },
    viper: { name: "VIPER", laserColor: "#ff2200", stats: { thrust: 7, fric: 0.98, fireRate: 0.15 }, 
        fire: () => { bullets.push(createBolt(0)); playSfx('shoot'); },
        draw: (ctx, r, thrusting) => {
            ctx.fillStyle = "#e0e0e0"; ctx.strokeStyle = "#333"; ctx.lineWidth = 1; ctx.fillRect(-r*1.2, -r*0.45, r*0.4, r*0.25); ctx.fillRect(-r*1.2, r*0.2, r*0.4, r*0.25); ctx.fillRect(-r*1.2, -r*0.12, r*0.4, r*0.24); 
            ...
            ctx.fillStyle = "#111"; ctx.beginPath(); ctx.ellipse(-r*0.1, 0, r*0.35, r*0.12, 0, 0, Math.PI*2); ctx.fill(); 
    ...
    nebuchadnezzar: { name: "NEBUCHADNEZZAR", laserColor: "#00aaff", stats: { thrust: 5, fric: 0.94, fireRate: 1.5 },
        fire: () => { 
            playSfx('nuke'); let empRange = canvas.width * 0.3;
            targets.forEach(t => { if (Math.hypot(ship.x - t.x, ship.y - t.y) < empRange) { if (t.hp !== undefined) t.hp -= 20; else t.hp = -1; } });
            particles.push({ x: ship.x, y: ship.y, xv: 0, yv: 0, life: 0.5, color: "#00aaff", size: empRange, isEmp: true });
        },
        draw: (ctx, r, thrusting) => {
            ctx.fillStyle = "#222529"; ctx.strokeStyle = "#445566"; ctx.lineWidth = 1; ctx.fillRect(-r, -r*0.3, r*1.5, r*0.6); ctx.strokeRect(-r, -r*0.3, r*1.5, r*0.6);
            ...
            ctx.fillStyle = "#111"; ctx.beginPath(); ctx.arc(-r*0.5, -r*0.5, r*0.3, 0, Math.PI*2); ctx.fill(); ctx.stroke(); ctx.beginPath(); ctx.arc(r*0.2, -r*0.5, r*0.3, 0, Math.PI*2); ctx.fill(); ctx.stroke();
    ...
    lightship: { name: "LIGHTSHIP", laserColor: "#00ffff", stats: { thrust: 8, fric: 0.99, fireRate: 0.3 },
        fire: () => { bullets.push(createBolt(0)); playSfx('shoot'); },
        draw: (ctx, r, thrusting) => {
            ctx.fillStyle = "#050505"; ctx.strokeStyle = "#00ffff"; ctx.lineWidth = 2;
    ...
    milano: { name: "MILANO", laserColor: "#ff9900", stats: { thrust: 7, fric: 0.96, fireRate: 0.2 },
        ...
        draw: (ctx, r, thrusting) => {
            ctx.fillStyle = "#1976d2"; ctx.strokeStyle = "#ff9900"; ctx.lineWidth = 1.5;
            ctx.beginPath(); ctx.moveTo(-r*0.5, 0); ctx.lineTo(-r*0.2, -r*1.2); ctx.lineTo(r*0.2, -r*1.2); ctx.lineTo(r*0.5, 0); ctx.fill(); ctx.stroke();
            ctx.beginPath(); ctx.moveTo(-r*0.5, 0); ctx.lineTo(-r*0.2, r*1.2); ctx.lineTo(r*0.2, r*1.2); ctx.lineTo(r*0.5, 0); ctx.fill(); ctx.stroke();
            ctx.fillStyle = "#ff9900"; ctx.beginPath(); ctx.moveTo(-r*0.8, -r*0.2); ctx.lineTo(r*1.2, 0); ctx.lineTo(-r*0.8, r*0.2); ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.fillStyle = "#111"; ctx.beginPath(); ctx.arc(r*0.2, 0, r*0.15, 0, Math.PI*2); ctx.fill(); 
            if (thrusting) { applyGlow(ctx, "#00aaff", 15); ctx.fillStyle = "#fff"; ctx.fillRect(-r*0.9, -r*0.1, 4, r*0.2); clearGlow(ctx); }
        }
    },
    fsociety: { name: "FSOCIETY", laserColor: "#00ff00", stats: { thrust: 6, fric: 0.97, fireRate: 0.4 },
...
const TargetDesigns = {
    ...
    asteroid: {
        draw: (ctx, r, t) => {
            let astGrad = ctx.createRadialGradient(r*0.3, -r*0.3, 0, 0, 0, r); astGrad.addColorStop(0, "#7a7263"); astGrad.addColorStop(0.5, "#4a4233"); astGrad.addColorStop(1, "#1a1814");
            ...
            ctx.strokeStyle = "rgba(0,0,0,0.4)"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(pts[0].x*0.5, pts[0].y*0.5); ctx.quadraticCurveTo(0, 0, pts[2].x*0.5, pts[2].y*0.5); ctx.stroke();
            if (t.craters) {
                t.craters.forEach(c => { 
                    let craterGrad = ctx.createRadialGradient(c.x, c.y, 0, c.x, c.y, c.r); craterGrad.addColorStop(0, "#1a1814"); craterGrad.addColorStop(1, "#3a3223");
                    ctx.fillStyle = craterGrad; ctx.beginPath(); ctx.arc(c.x, c.y, c.r, 0, Math.PI*2); ctx.fill(); ctx.strokeStyle = "#111"; ctx.lineWidth = 1.5; ctx.stroke();
                    ctx.strokeStyle = "rgba(255,255,255,0.1)"; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(c.x, c.y, c.r, 0, Math.PI); ctx.stroke(); 
                });
            }
        }
    }
...
function startLevel() {
    targets = []; powerups = []; enemyBullets = []; lightTrails = []; powerupSpawnedThisLevel = false;
...
function damagePlayer(amt) {
    ...
    if (playerShield > 0) {
        playerShield -= amt;
        if (playerShield < 0) { playerHp += playerShield; playerShield = 0; playSfx('boom'); } 
    } else { playerHp -= amt; }
...
function update(dt) {
    ...
    if (ship.thrusting) { ship.xv += stats.thrust * Math.cos(ship.angle) * dt; ship.yv += stats.thrust * Math.sin(ship.angle) * dt; }
    ...
    if ((keys["Space"] || mouse.leftDown) && fireCooldown <= 0) {
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
        let p = particles[i]; p.x += p.xv; p.y += p.yv; p.life -= dt * 2.0; 
        if (p.isEmp) p.size += dt * 800; 
    ...
    for (let i = powerups.length - 1; i >= 0; i--) {
        ...
        if (Math.hypot(ship.x - p.x, ship.y - p.y) < ship.r + p.r) { 
            playSfx('powerup');
            if(p.type === 'M') multishotTimer = 10.0; else if (p.type === 'S') playerShield = playerMaxShield; else if (p.type === 'H') playerHp = playerMaxHp; else if (p.type === 'B') bombs++; 
            score += 150; updateUI(); powerups.splice(i, 1); 
        }
    }
    ...
    targets.forEach(t => { 
        ...
        if (t.type === "sentinel") {
            let angToPlayer = Math.atan2(ship.y - t.y, ship.x - t.x); t.angle = angToPlayer; t.xv = Math.cos(angToPlayer) * 3; t.yv = Math.sin(angToPlayer) * 3;
        }
        if (t.type === "tie_advanced" && t.fireTimer !== undefined) {
            t.fireTimer -= dt;
            if (t.fireTimer <= 0) {
                let angToPlayer = Math.atan2(ship.y - t.y, ship.x - t.x); enemyBullets.push({ x: t.x, y: t.y, xv: 6 * Math.cos(angToPlayer), yv: 6 * Math.sin(angToPlayer), range: canvas.width * 0.8 });
                playSfx('enemyShoot'); t.fireTimer = 2.0;
            }
        }
        if (t.type === "boss_station") {
            ...
            t.chargeTimer += dt;
            if (t.chargeTimer > 5.0) {
                playSfx('boom'); shake += 10; t.chargeTimer = 0; let angToPlayer = Math.atan2(ship.y - t.y, ship.x - t.x);
                for(let i=0; i<5; i++) enemyBullets.push({ x: t.x, y: t.y, xv: 8 * Math.cos(angToPlayer+(i*0.1-0.2)), yv: 8 * Math.sin(angToPlayer+(i*0.1-0.2)), range: canvas.width });
            }
        }
        ...
        if (t1.type !== "asteroid" && !t1.type.startsWith("boss")) {
            ...
            for (let j = 0; j < targets.length; j++) {
                ...
                if (t2.type === "asteroid") {
                    ...
                    if (dist < t1.r + t2.r * 0.8) { smashed = true; spawnParticles(t1.x, t1.y, "#ff5500", 20); playSfx('boom'); score += 10; updateUI(); break; }
                }
            }
    ...
    for (let i = targets.length - 1; i >= 0; i--) {
        ...
        if (hitByTrail) { if (targets[i].hp !== undefined) targets[i].hp -= 0.5; else targets[i].hp = -1; targets[i].hitFlash = 0.1; }
    }
    ...
    for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        for (let j = targets.length - 1; j >= 0; j--) {
            ...
            if (t.type === "boss_mothership") {
                ...
                t.nodes.forEach(n => {
                    if (n.hp > 0) {
                        allDead = false; let nx = t.x + Math.cos(n.ang) * t.r * 1.2; let ny = t.y + Math.sin(n.ang) * t.r * 1.2;
                        if (Math.hypot(bullets[i].x - nx, bullets[i].y - ny) < 20) { n.hp--; hitNode = true; spawnParticles(nx, ny, "#0ff", 5); playSfx('hit'); }
                    }
                });
            ...
            if (Math.hypot(bullets[i].x - t.x, bullets[i].y - t.y) < (t.r + (bullets[i].r || 2))) {
                if (bullets[i].isHack) { t.stunned = 3.0; hit = true; playSfx('powerup'); break; } 
                
                playSfx('hit');
                if (t.hp !== undefined && t.hp > 1) { t.hp--; t.hitFlash = 0.1; score += 25; updateUI(); hit = true; spawnParticles(bullets[i].x, bullets[i].y, "#fff", 5); break; }
                
                playSfx('boom'); shake = t.r > 30 ? 10 : 3;
                spawnParticles(t.x, t.y, t.type==="asteroid" ? "#aaa" : "#ff5500", t.r > 30 ? 30 : 15);
                
                let diffMod = 1 + (level * 0.1);
                if (t.type === "boss_station" || t.type === "boss_mothership") { score += 1500; shake = 25; for(let k=0; k<6; k++) spawnTarget("asteroid", 30, diffMod * 1.5, t.x, t.y); } 
                else if (t.type === "star_destroyer") { score += 100; spawnTarget("tie_interceptor", 25, diffMod * 1.2, t.x, t.y); spawnTarget("tie_interceptor", 25, diffMod * 1.2, t.x, t.y); } 
                else if (t.type === "tie_interceptor" || t.type === "tie_advanced") { score += 50; spawnTarget("tie_fighter", 15, diffMod * 1.5, t.x, t.y); spawnTarget("tie_fighter", 15, diffMod * 1.5, t.x, t.y); } 
                else if (t.type === "tie_fighter" || t.type === "sentinel") { score += 25; } 
                else if (t.type === "asteroid") { score += (t.r > 20) ? 20 : 50; if (t.r > 20) { spawnTarget("asteroid", t.r / 2, diffMod * 1.3, t.x, t.y); spawnTarget("asteroid", t.r / 2, diffMod * 1.3, t.x, t.y); } }
    ...
    if (targets.length === 0 && gameState === "PLAYING") { 
        if (level >= 25) { checkAndSaveScore(); gameState = "VICTORY"; } 
        else { level++; updateUI(); gameState = "LEVEL_TRANSITION"; bullets = []; enemyBullets = []; lightTrails = []; hyperspace = 0; playSfx('powerup'); } 
    }
}
...
function render() {
    ...
    ctx.fillStyle = "white";
    for(let i=0; i<60; i++) { 
        let sx = (Math.sin(i*74) * 10000 + frames * (i%3)) % canvas.width; let sy = (Math.cos(i*31) * 10000 + frames * (i%2)) % canvas.height; 
        if (gameState === "LEVEL_TRANSITION") { ctx.fillRect(Math.abs(sx), Math.abs(sy), hyperspace*50, 1); } 
        else { ctx.globalAlpha = (i % 3 === 0) ? 0.3 : 0.8; ctx.fillRect(Math.abs(sx), Math.abs(sy), i%2===0?2:1, i%2===0?2:1); ctx.globalAlpha = 1.0; }
    }
